...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/login', () => {
    test('should return 401 You are not allowed to login! You need to confirm your email', async () => {
      await insertUsers([userOne]);
      const loginCredential = {
        email: userOne.email,
        password: userOne.password,
      };
      await request(app).post('/v1/auth/login').send(loginCredential).expect(httpStatus.UNAUTHORIZED);
    });
